...
class OnlineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print("labels_ori shape:", gt.shape, "unique values:", torch.unique(gt))
        ...
        # 转换为torch张量并调整图像形状为 [1, D, H, W]
        if len(gt.shape) == 3:
            gt = torch.tensor(gt, dtype=torch.float32)  # 形状: [height, width, depth]
            gt = torch.unsqueeze(gt, 0)  # 添加通道维度: [1, height, width, depth]
            gt = gt.permute(0, 3, 1, 2)  # 调整为 [1, depth, height, width]
        elif len(gt.shape) == 4 and gt.shape[0] != 1:
            raise ValueError(f"预期图像具有1个通道，实际形状为 {gt.shape}")
        ...
        # print("label shape:", sample['label'].shape, "label values:", torch.unique(sample['label']))
        ...
        # print("labels_ori shape:", sample['label'].shape, "unique values:", torch.unique(sample['label']))
        # print("labels_ori shape:", sample['ori_label'].shape, "unique values:", torch.unique(sample['ori_label']))

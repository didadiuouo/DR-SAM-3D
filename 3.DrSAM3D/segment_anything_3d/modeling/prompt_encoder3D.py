...
class PromptEncoder3D(nn.Module):
    ...
    def __init__(
        ...
        self.mask_downscaling = nn.Sequential(
            nn.Conv3d(1, 4, kernel_size=2, stride=2),  # [4, 1, 2, 2, 2]
            LayerNorm3d(4),  # [4]
            activation(),
            nn.Conv3d(4, 16, kernel_size=2, stride=2),  # [16, 4, 2, 2, 2]
            LayerNorm3d(16),  # [16]
            activation(),
            nn.Conv3d(16, embed_dim, kernel_size=1),  # [384, 16, 1, 1, 1]
        )
    ...
    def _embed_masks(self, masks: torch.Tensor) -> torch.Tensor:
        ...
        if masks.shape[1] == 3:
            masks = masks[:, 0:1, :, :, :]  # 取第一个通道
        elif masks.shape[1] == 16:
            masks = masks[:, 0:1, :, :]  # 取第一个通道，2D情况
            masks = masks.unsqueeze(-1)  # 添加 z 维度，变为 [1, 1, 32, 32, 1]
            masks = masks.expand(-1, -1, -1, -1, 32)  # 扩展 z 维度到 32

        mask_embedding = self.mask_downscaling(masks)
        target_size = (8, 8, 8)
        if mask_embedding.shape[2:] != target_size:
            mask_embedding = F.adaptive_avg_pool3d(mask_embedding, target_size)

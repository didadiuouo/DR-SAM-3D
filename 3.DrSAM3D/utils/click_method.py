...
def get_next_click3D_torch_2(prev_seg, gt_semantic_seg):
   ...
   #     pred_masks = pred_masks[:, :true_masks.shape[1], :, :, :]  # 选择前 16 个通道
   ...
   if pred_masks.shape != true_masks.shape:
       ...
       pred_masks_expanded = torch.zeros(batch_size, 16, 128, 128, device=pred_masks.device)
       # 选择第一个通道赋值
       pred_masks_expanded = pred_masks[:, 0, :, :, :]
       # 打印调整后的形状
       # print(f"pred_masks shape after adjustment: {pred_masks_expanded.shape}")
       # print(f"true_masks shape: {true_masks.shape}")
   else:
       pred_masks_expanded = pred_masks

   fn_masks = torch.logical_and(true_masks, torch.logical_not(pred_masks_expanded))
   fp_masks = torch.logical_and(torch.logical_not(true_masks), pred_masks_expanded)
   to_point_mask = torch.logical_or(fn_masks, fp_masks)

   # 统一维度处理
   if fn_masks.dim() == 5:  # [batch, channels, depth, height, width]
       fn_masks = fn_masks.any(dim=1)  # 合并通道维度
       to_point_mask = to_point_mask.any(dim=1)
   elif fn_masks.dim() == 4:  # [batch, depth, height, width] or [batch, 1, height, width]
       fn_masks = fn_masks.squeeze(1) if fn_masks.shape[1] == 1 else fn_masks.any(dim=1)
       to_point_mask = to_point_mask.squeeze(1) if to_point_mask.shape[1] == 1 else to_point_mask.any(dim=1)

   for i in range(gt_semantic_seg.shape[0]):
       points = torch.argwhere(to_point_mask[i])  # 获取非零点坐标
       if len(points) == 0:
           continue
       point = points[np.random.randint(len(points))]  # 随机选择一个点

       # 根据 fn_masks[i] 的维度选择索引
       if fn_masks[i].dim() == 2:  # [height, width]
           is_positive = fn_masks[i, point[-2], point[-1]].item()
           point = torch.tensor([0, point[-2], point[-1]], device=point.device)
       elif fn_masks[i].dim() == 3:  # [depth, height, width]
           is_positive = fn_masks[i, point[-3], point[-2], point[-1]].item()
           point = point[-3:] if point.shape[0] > 3 else point
       else:
           raise ValueError(f"不支持的 fn_masks[{i}] 维度: {fn_masks[i].shape}")

       bp = point.float().reshape(1, 1, 3)  # 直接重塑坐标

...
def data_preprocess(subject, meta_info, category_index, target_spacing, crop_size=880):
    ...
    crop_transform = tio.CropOrPad(mask_name='label', target_shape=(12, crop_size, crop_size))
...
def data_postprocess(roi_pred_numpy, meta_info):
    ...
    # roi_pred_tensor = roi_pred_numpy.squeeze(1).detach()
    roi_pred_tensor = torch.from_numpy(roi_pred_numpy.astype(np.float32)).squeeze(1)
...
def validate_paired_img_gt(model, net, img_path, gt_path, valid_datasets, output_path, num_clicks=1, crop_size=128, target_spacing=(1, 1, 1), seed=233):
    ...
    for category_index in exist_categories:
        ...
        roi_img, roi_label, meta_info = data_preprocess(category_specific_subject,
                                                          category_specific_meta_info,
                                                          category_index=category_index,
                                                          target_spacing=target_spacing,
                                                          crop_size=crop_size)

        inputs = roi_img.to(device="cpu")
        ...
        labels_box = misc.masks_to_boxes(roi_label[:, 0, :, :])
        try:
            labels_points = misc.masks_sample_points(roi_label[:, 0, :, :, :])  # 3D 点 [B, N, 3]
        except:
            input_keys = ['box', 'noise_mask']
            # input_keys = ['noise_mask']
        labels_512 = F.interpolate(roi_label.float(), size=(256, 256, 256), mode='nearest').long()
        ...
        batched_output, hierarchical_embeddings = model(batched_input, multimask_output=False)
        ...
        masks_sam, roi_pred_numpy = net(image_embeddings=encoder_embedding, image_pe=image_pe,
                                  sparse_prompt_embeddings=sparse_embeddings,
                                  dense_prompt_embeddings=dense_embeddings, multimask_output=False,
                                  med_token_only=False, hierarchical_embeddings=hierarchical_embeddings, )

        medsam_seg_prob = torch.sigmoid(roi_pred_numpy)

        # print("masks_dr shape:", roi_pred_numpy.shape, "unique sigmoid values:",
        #       torch.unique(roi_pred_numpy))

        medsam_seg_prob = medsam_seg_prob.detach().numpy()
        roi_pred_numpy = (medsam_seg_prob > 0.3).astype(np.uint8)

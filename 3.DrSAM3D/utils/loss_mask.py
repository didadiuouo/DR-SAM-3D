...
def get_uncertain_point_coords_with_randomness(
    ...
    uniform_indices= uniform_indices.to('cuda:3')
    indices = torch.cat([uniform_indices, importance_indices], dim=1)[:, :num_points]
    # 转换为3D坐标
    coords = torch.stack(
        [
            indices // (h * w),  # 深度
...
def loss_masks(src_masks, target_masks, num_masks, oversample_ratio=1.0, num_classes=1):
    """

...
def compute_metrics_drsam(gt_value: str,
                    pred_value: str,
    ...
    if not os.path.exists(gt_value):
        raise FileNotFoundError(f"Ground truth file not found: {gt_value}")
    if not os.path.exists(pred_value):
        raise FileNotFoundError(f"Prediction file not found: {pred_value}")
    ...
    try:
        gt_nii = nib.load(gt_value)
        pred_nii = nib.load(pred_value)
        ...
        if classes is None:
            ...
            if not determined_classes:
                print(f"Warning: No non-zero classes found in ground truth file: {gt_value}")
                return dict(results)
        else:
        ...
        for i in determined_classes:
            ...
            try:
                ...
                print(
                    f"Warning: Error computing metrics for class {i} : {metric_error}"
                )
        ...
        print(f"Error processing files 1 and 2: {str(e)}")
        # Return partially filled results or an empty dict if critical error
        return dict(results) if results else {}

    return dict(results)

def print_computed_metrics(results_data: Dict[Any, Any], avg_only=False, title: str = "Computed Metrics", gt_path = ''):
    """
    ...
    print(f"--- End of Metrics : {gt_path}---")
